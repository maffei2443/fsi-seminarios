...
#retirando a coluna do algoritmo kNN, criando o atributo "Class", que corresponde ao algortimo com melhor acurácia
# para um dado objeto e excluindo as colunas referentes à métrica acc (depois de atribuidas as classes).
#Distribuição das classes na base: {'RF.acc': 162, 'SVM.acc': 101, 'ANN.acc': 67, 'C4.5.acc': 55}
...
data = data.drop(["ANN.f1m", "ANN.auc", "C4.5.f1m", "C4.5.auc", "kNN.acc", "kNN.f1m", "kNN.auc", "SVM.f1m", "SVM.auc", "RF.f1m", "RF.auc"],axis = 1)
...
for i in data.index:
    maior = max(data.loc[i, ['ANN.acc', 'C4.5.acc', 'SVM.acc', 'RF.acc']])
    for j in ['ANN.acc', 'C4.5.acc', 'SVM.acc', 'RF.acc']:
...
data = data.drop(['ANN.acc', 'C4.5.acc', 'SVM.acc', 'RF.acc'], axis =1)
